...
class JMXItemChecker extends ItemChecker
{
	...
	public JMXItemChecker(JSONObject request) throws ZabbixException
	{
		...
		try
		{
			...
			if (conn.startsWith("weblogic:")) {
				String[] host = conn.split(":");
				url = new JMXServiceURL("service:jmx:rmi:///jndi/iiop://" + host[1] + ":" + port + "/weblogic.management.mbeanservers.domainruntime");
			} else {
	...
	public JSONArray getValues() throws ZabbixException
	{
		...
		try
		{
			if (url.getURLPath().startsWith("service:jmx:rmi:///jndi/iiop://"))
			{
				...
				env.put(ZabbixJMXConnectorFactory.PROTOCOL_PROVIDER_PACKAGES, "weblogic.management.remote");
